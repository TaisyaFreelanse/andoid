...
- Проверки доменов через Semrush API
...
- **Функции:**
  ...
  - WebSocket для real-time обновлений (статусы, логи)
  - WebSocket для стриминга экранов устройств в реальном времени
...
- **Функции:**
  ...
  - Стриминг экрана устройства в реальном времени (опционально)
...
- **Функции:**
  ...
  - Сетка устройств с экранами в реальном времени (стриминг)
...
Android Agent → GET /api/agent/tasks (polling)
 ...
 3. Парсинг adurl и проверка Semrush

Android Agent → Парсинг страницы, извлечение adurl
              → POST /api/agent/tasks/:id/result (с ad_url)
              → Backend извлекает домен из ad_url
              → Проверка наличия в semrush_data (кеш)
              → Если нет → запрос к Semrush API
              → Сохранение в semrush_data и parsed_data
...
- Автоматическая установка таймзоны устройства в соответствии с геолокацией прокси
  - При добавлении прокси определяется страна/таймзона (через GeoIP API или вручную)
  - При назначении прокси устройству автоматически устанавливается соответствующая таймзона
  - Это обеспечивает соответствие таймзоны устройства геолокации прокси
