...
- [ ] `/api/devices/:id/stream/start` - запуск стриминга экрана
- [ ] `/api/devices/:id/stream/stop` - остановка стриминга экрана
...
- [ ] `/api/agent/stream` - отправка скриншота для стриминга (WebSocket)

 Services
- [ ] `device.service.ts` - бизнес-логика устройств
- [ ] `task.service.ts` - бизнес-логика задач
- [ ] `proxy.service.ts` - управление прокси
  - [ ] Определение геолокации прокси (страна/таймзона) через GeoIP API
  - [ ] Сохранение геолокации прокси в БД
  - [ ] Отправка команды агенту на установку таймзоны при назначении прокси
- [ ] `semrush.service.ts` - интеграция с Semrush API
- [ ] `storage.service.ts` - работа с MinIO/S3
- [ ] `streaming.service.ts` - стриминг экранов устройств (WebSocket)
...
- [ ] WebSocket сервер для стриминга экранов
  - [ ] Обработка подключений от агентов
  - [ ] Ретрансляция скриншотов к UI клиентам
  - [ ] Управление подключениями (start/stop стриминга)
...
- [ ] `/devices/grid` - сетка устройств с экранами в реальном времени
- [ ] `/tasks` - список задач
...
- [ ] `DeviceGrid` - сетка устройств с экранами (4x5 для 20 устройств)
- [ ] `DeviceScreen` - отображение экрана одного устройства
...
- [ ] Стриминг экранов устройств в реальном времени (WebSocket)
- [ ] Сетка устройств с обновлением экранов каждые 1–2 секунды
...
- [ ] `UniquenessService.kt` - уникализация устройства
  ...
  - [ ] Изменение timezone
  - [ ] Изменение геолокации (GPS координаты через mock location)
  - [ ] Изменение build.prop
- [ ] `ScreenCaptureService.kt` - захват и стриминг экрана
  - [ ] Захват скриншотов через MediaProjection или root
  - [ ] Оптимизация изображений (JPEG сжатие, уменьшение разрешения)
...
- [ ] `ProxyManager.kt` - управление прокси
  ...
  - [ ] Автоматическая установка таймзоны при смене прокси (в соответствии с геолокацией прокси)
...
- [ ] Стриминг экранов устройств работает (20 устройств одновременно)
